...
export const getTwitchStreamer = (req, response) => {
    async function userRequest(accessToken){

        const userOptions = {
            url: 'https://api.twitch.tv/helix/users?login=itsRyanHiga',
            method: 'GET',
            headers: {
        ...
        if(!accessToken){
            ...
            console.log("GameOption", userOptions);
    ...
    const mainRequest = request.post(options, async (err, res, body) => {
        ...
        return await userRequest(body.access_token);
    });
};
